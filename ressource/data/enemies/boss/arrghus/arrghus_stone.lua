...
function enemy:on_created()
  self:set_life(10)
  self:set_damage(4)
  local sprite = self:create_sprite("enemies/boss/arrghus/arrghus_stone")
  self:set_size(80, 80)
  self:set_origin(40, 72)
  self:set_hurt_style("boss")
  self:set_attack_consequence("sword", "protected")
  self:set_attack_consequence("arrow", "protected") -- Ajout de cette ligne pour protéger contre les flèches
...
function enemy:check_action()
  if self:get_life() > 4 then
    ...
    self:create_enemy({ breed = "boss/arrghus/arrghus_baby", treasure_name = "fleches" })
...
function enemy:close()
  ...
  self:create_enemy({ breed = "boss/arrghus/arrghus_baby", treasure_name = "arrow" })
