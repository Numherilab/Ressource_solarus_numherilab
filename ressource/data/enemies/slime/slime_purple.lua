...
function enemy:on_created()
  ...
  left_wing = wings:create_sprite("enemies/slime/wings_evil")
  right_wing = wings:create_sprite("enemies/slime/wings_evil")
  ...
  sol.timer.start(self, 50, function()
    if shift == 0 then
      ...
      local slime = self:get_map():create_enemy({x=x, y=y, layer=layer, direction = 0, 
        breed = "slime/slime_green"})
      slime:replace_sprite("enemies/slime/slime_purple")
...
function enemy:create_egg()
  ...
  local x, y, layer = self:get_position()
  local prop = {x = x, y = y, layer = layer, direction = 0, breed = "slime/slime_egg"}
  local egg = map:create_enemy(prop)
  egg:set_slime_model("slime/slime_purple")
...
function enemy:start_wind()
  state = "wind"
  local wind = wings:create_sprite("enemies/slime/slime_purple", "wind")
