...
function enemy:on_created()
  ...
  if not sprite then sprite = self:create_sprite("enemies/" .. self:get_breed()) end
  state = "hidden"
...
function enemy:throw_fireball()
  ...
  local prop = {x = x, y = y, layer = layer, direction = 0,
    width = 8, height = 8, sprite = "enemies/slime/slime_red"}
...
function enemy:create_flame(x, y, layer)
  local prop = {x = x, y = y, layer = layer, direction = 0, breed = "projectile/flame"}
...
function enemy:create_egg()
  ...
  egg:set_slime_model("slime/slime_red")
