...
function enemy:go(direction)
  ...
  enemy.fuse_timer = sol.timer.start(enemy, FUSE_LENGTH, function() enemy:split() end)
...
function enemy:split()
  ...
  for i=1, NUM_CHILDREN do
        ...
        breed = "projectile/energy_ball_small_black"
      })
...
function enemy:on_custom_attack_received(attack, sprite)
  if attack == "sword" then
    ...
    if enemy.fuse_timer then enemy.fuse_timer:stop() end
  end
end