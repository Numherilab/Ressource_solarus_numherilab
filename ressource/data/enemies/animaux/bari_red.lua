...
function enemy:on_created()
  self:set_life(3)
  self:set_damage(12)
  self:create_sprite("enemies/animaux/bari_red")
  self:set_size(16, 16)
...
function enemy:shock()
  ...
  if sprite:has_animation("shaking") then
    ...
    print("Animation 'shaking' does not exist in sprite 'enemies/animaux/bari_red'")
  end
end
...
function enemy:stop_shock()
  ...
  if sprite:has_animation("walking") then
    ...
    print("Animation 'walking' does not exist in sprite 'enemies/animaux/bari_red'")
  end
end
...
function enemy:on_hurt_by_sword(hero, enemy_sprite)
  if shocking then
    hero:start_hurt(20) -- Inflige des dégâts au héros en le touchant pendant l'électrocution
  else
    self:hurt(1)
    self:remove_life(1)
  end
